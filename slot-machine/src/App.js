...
function App() {
  ...
  const ip = "http://192.168.68.96:"; // Replace with correct IP address
  ...
  const getScores = async () => {
    try {
      const dbResponse = await fetch(`${ip}3001/getScore?minigame=SlotMachine,bigger`);
  ...
  const AddScore = async () => {
    try {
      ...
      const res = await fetch(`${ip}3001/addScoreSum?score=${userName},SlotMachine,${winsSinceLastSave}`);
